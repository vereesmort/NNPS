...
#------------------------------------------------------------------------
#construct drug features (using only protein targets, without mono side effects)
pca_ = PCA(.95)
pca_.fit(drug_protein_adj)
drug_feat = pca_.transform(drug_protein_adj)
#------------------------------------------------------------------------
...
for k in range(n_drugdrug_rel_types):
    ...
    mcc.append(mc)
model.save('NNPS_without_mono.h5')
loaded_model =load_model('NNPS_without_mono.h5')
...
results_df.to_csv("Results/NNPS_without_mono_30_results.csv", index=False)
