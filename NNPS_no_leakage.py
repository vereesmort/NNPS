...
# FIXED: Only extract upper triangle (i < j) to avoid duplicate pairs
# 
# ISSUE FIXED:
# - Original code iterated over all (i,j) pairs, creating duplicates:
#   Each undirected edge appeared twice as [i,j] and [j,i]
# - This caused the same pair to potentially end up in different splits (data leakage)
# 
# SOLUTION:
# - Only extract pairs where i < j (upper triangle of symmetric matrix)
# - This ensures each unique pair appears exactly once
# - Feature construction (drug_feat[i[0]] + drug_feat[i[1]]) is order-invariant
#   since addition is commutative, so consistent ordering is maintained
edges=[]
for k in range(n_drugdrug_rel_types):
    l=[]
    # Only consider upper triangle to avoid duplicates (order-invariant)
    for i in range(n_drugs):
        for j in range(i+1, n_drugs):  # Changed: j starts from i+1 instead of 0
