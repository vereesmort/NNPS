...
pca = PCA(.95)
pca.fit(drug_label)
drug_feat = pca.transform(drug_label)
#------------------------------------------------------------------------
...
for k in range(n_drugdrug_rel_types):
    ...
    mcc.append(mc)
model.save('NNPS_only_mono.h5')
loaded_model =load_model('NNPS_only_mono.h5')
...
results_df.to_csv("Results/NNPS_only_mono_30_results.csv", index=False)
